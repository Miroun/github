...
class ImgModel(nn.Module):
    def __init__(self, input_channel=3):
        ...
        if input_channel != 3:
            self.back[0] = nn.Conv2d(
                input_channel, 64, kernel_size=3, padding=1)
...
class MultiHeadModel(nn.Module):
    ...
    def forward(self, x, device):
        ...
        ele_src_feat = x[4 * bs:]
        aero_diff_out = self._aero_diff_fc(
            aero_diff_feat.reshape(bs, -1)).reshape(-1)
        ele_diff_out = self._ele_diff_fc(
            ele_diff_feat.reshape(bs, -1)).reshape(-1)
